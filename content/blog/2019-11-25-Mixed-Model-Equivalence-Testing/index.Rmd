...
paste("Power =", power)
```
We can verify our calculations against the results from the `power_t_TOST()` function in TOSTER.
...
simPower <- function(es = 0, s = 1, n = sample(10:100, 1), alpha = 0.05, bound_l = -0.3, bound_u = 0.3) {
  ...
  invisible(capture.output(pow_toster <- TOSTER::power_t_TOST(alpha = alpha, n = n, low_eqbound = bound_l, high_eqbound = bound_u)))
  c(pow_ncp, pow_toster[["power"]], s, n)
...
plot(sim[,1], sim[,2], xlab = "ncp power", ylab = "power_t_TOST")
