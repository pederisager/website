---
title: "Why does correlation not equal causation?"
...
header:
  caption: ''
  image: 'img/consider_all_the_causal_relations.png'
categories: []
slug: why_does_correlation_not_equal_causation
tags: 
- causal inference
- causal graphs
- correlation
summary: "This blog post is a beginners guide to the relationship between causation and correlation. In it, I will explain why it makes sense to think about causal explanations for observed correlation, why correlation does not always imply causation, and which alternative causal models to consider when you are trying to figure out why two variables in your data are correlated. The blog post is intended as a non-technical introduction. I use only words and pictures to explain all concepts and logic, and I use a running example from health psychology to illustrate. The post should be relevant to anyone who is interested in using data to understand causal mechanisms."
---


# Abstract

This blog post is a beginners guide to the relationship between causation and correlation. In it, I will explain why it makes sense to think about causal explanations for observed correlation, why correlation does not always imply causation, and which alternative causal models to consider when you are trying to figure out why two variables in your data are correlated. The blog post is intended as a non-technical introduction. I use only words and pictures to explain all concepts and logic, and I use a running example from health psychology to illustrate. The post should be relevant to anyone who is interested in using data to understand causal mechanisms.

# Introduction
Most who have studied or worked in science will have heard the warning phrase “correlation does not imply causation”. The warning is usually accompanied by striking examples like the correlation between [ice cream sales and drowning deaths](https://www.google.com/search?sca_esv=581520105&rlz=1C1ONGR_noNO1074NO1074&sxsrf=AM9HkKmgd7QVHKUriPudUWxRHAk0tACM_Q:1699712017827&q=correlation+between+ice+cream+sales+and+drowning&tbm=isch&source=lnms&sa=X&sqi=2&ved=2ahUKEwiP46vYkLyCAxXeAxAIHY3qBxoQ0pQJegQIDBAB&biw=1920&bih=923&dpr=1#imgrc=4UgPCI9EnIL8hM) through the calendar year, as shown below:

![drowning deaths and ice cream sales throughout the year](img/ice_cream_sales_drowning_deaths.png)
...
![](img/ice_cream_in_the_sun.jpg){width=50%}
...
![](img/exercise_mood_correlated.svg)
...
To see why, flip the problem around and consider what causation implies. If physical exercise causes changes in mood then a person's mood will vary in tune with their exercise regime. People who exercise a lot will also report a high mood because exercise leads to a high mood. People who do not exercise will report a lower mood because they are not benefiting from the mood-boosting effects of exercise. When we look at a collection of people who vary in how much they exercise, we should therefore expect exercise scores to correlate with reported mood.

![](img/exercise_causes_mood.svg)
...
# Correlation does not imply your favored causal direction
...
It is sometimes also possible that both theories are true! Exercise could put you in a better mood *and* being in a better mood could make you more inclined to go exercise:

![](img/exercise_mood_cycle.svg)
...
# Correlation does not equal a direct causal connection
...
![](img/stress_exercise_mood_confounding.svg)
...
![](img/weather_icecream_drowning_confounding.svg)

# Selection bias

Another way two variables can become correlated without any direct causal link is through selection bias. Selection happens when the variables we are interested in both cause a third variable, and that third variable determines who ends up in our dataset (also referred to as [conditioning on a collider](https://www.bmj.com/content/381/bmj.p1135)). Left alone, collider structures do not cause X and Y to become correlated. But things change if we start selecting data points based on the shared outcome variable. 
...
* **Mood bias:** The worse mood someone is in when asked to participate, the less they want to participate.
* **Exercise bias:** The survey was conducted at a local gym. People who work out frequently tend to be really focused during each workout and do not like to be disturbed. People who work out infrequently tend to wander while at the gym, and don’t mind being disturbed as much. Thus, the more someone works out, the less they want to participate. 

![](img/exercise_mood_desire_selection_bias.svg)
...
![](img/selection_bias_scatterplot.svg)
...
# Correlation implies one of the following… 
...
1. A causes B
1. B causes A
1. A causes B and B causes A
1. A and B are both caused by C
1. A and B both cause D, and D is selected on

![](img/alternative_causal_models.svg)
...
![](img/all_causal_models_at_once.svg)
...
1. We can *imagine* causal models and then ask what *empirical observations* we should expect if a particular causal model is true. For example: “If exercise causes improvements in mood, what correlation/association should we expect to see between exercise and mood in our study?”. This kind of thinking lets us design studies to empirically test causal models we are interested in. 
2. We can look at *empirical observations* and then *imagine* what causal models could have produced those data. For example: “If there is a correlation between exercise and mood, what causal models are compatible with that observation?”. This kind of thinking helps us realize what causal models are interesting to begin with. 
...
![](img/consider_all_the_causal_relations.png)
